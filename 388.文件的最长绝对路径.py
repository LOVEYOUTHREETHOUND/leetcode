...
class Solution:
    def lengthLongestPath(self, input: str) -> int:
        # 使用栈对当前遍历对象的父路径的每个部分的长度进行保存:
        stk = []
        # 维护“文件”的最长路径长度：
        max_length = 0
        # 遍历每一行(每一行对应的是一个目录或者一个文件，对于每一个部分，需要1、使栈中只保留其父路径的部分；2、将本身长度压入栈；3、如果是文件，需要更新最大长度)：
        for part in input.split('\n'):
          # 获取当前part的 前面的父路径的part数目(也就是制表符数目)：
          tabs = part.rfind('\t') + 1
          # 更新栈的层数（栈中应该有tabs层）：
          while len(stk) > tabs:
            stk.pop()
          # 将当前part push入栈，现在栈中是一个完整的路径：
          stk.append(len(part) - tabs)
          # 如果是文件，需要更新最大文件路径长度：
          if '.' in part:
